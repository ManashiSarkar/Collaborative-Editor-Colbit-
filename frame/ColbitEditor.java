	...
	private void output()
	{
		...
		try
		{
			...
			ChangeListener change = new ChangeListener()
			{
				...
				{
			        //Caret source = (Caret)e.getSource();

					int caretposition = caret.getDot();
			        System.out.println("caret at = " + caretposition);

					SwingWorker caretworker = new SwingWorker() 
					{
						protected String doInBackground() throws InterruptedException 
						{
							// stuff to be run on a separate thread
							...
							try
							{
								synchronized(versionDataOld)
								{
									synchronized(versionNumOld)
									{
										...
										// send versionnum and caretposition
										// receive sent versionnum and caretposition in input
										// store caretposition on server
										// on server, update Data for each opened file on each update, for new users.
									}
								}
							}
							catch(Exception e)
			...
			caret.addChangeListener(change);
